...
interface WhatsAppIconProps extends React.SVGProps<SVGSVGElement> {
  size?: number;
  className?: string;
  color?: "green" | "currentColor" | "white";
}

/**
 * Authentic WhatsApp vector icon.
 * Supports currentColor, brand green (#25D366), or white.
 */
...
/**
 * Full-color circular WhatsApp badge.
 */
export function WhatsAppBadge({
  ...
  const iconPadding = Math.max(2, Math.round(size * 0.18));
  const innerSize = size - iconPadding * 2;

  return (
    <div
      style={{ width: size, height: size }}
      className={`inline-flex items-center justify-center rounded-full bg-[#25D366] text-white shrink-0 select-none shadow-xs ${className}`}
      title="WhatsApp"
    >
      <WhatsAppIcon size={innerSize} color="white" />
    </div>
  );
}
