 ...
 * from the service role key to guarantee safe deterministic encryption.
 */
function getVaultKey(): Buffer {
  ...
  // Safe fallback: derive 32-byte key from SUPABASE_SERVICE_ROLE_KEY
  const fallbackSource = process.env.SUPABASE_SERVICE_ROLE_KEY || "atom-and-echo-default-vault-key-2026";
...
export function decryptPassword(encryptedPayload: string): string {
  ...
  try {
    ...
    console.error("Failed to decrypt password payload:", err.message);
